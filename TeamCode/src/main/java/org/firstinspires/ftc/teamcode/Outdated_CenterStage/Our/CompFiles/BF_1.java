...
@Autonomous(name="BF_1", group="Linear OpMode")

@Disabled
public class BF_1 extends LinearOpMode {
    ...
    public Action RaiseA() {
        ...
        return new Action() {
            @Override
            public boolean run(@NonNull TelemetryPacket telemetryPacket) {
                shoulder.setTargetPosition(-195);
                shoulder.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                shoulder.setPower(1);
    ...
    */
    ...
    public void runOpMode() {
        ...
        MecanumDrive drive = new MecanumDrive(hardwareMap, new Pose2d(-38, 63.4, 3 * Math.PI / 2));
...
/*
            if (cv.propLoc== OpenCVB.PropLoc.Left) {

                Actions.runBlocking(drive.mirroredActionBuilder(new Pose2d(-38, -64, Math.PI/2))
                        .splineToLinearHeading(new Pose2d(-32.5,-33,Math.PI),0)
                        .stopAndAdd(Servo())
                                .waitSeconds(.1)
                        .lineToX(-38)
                                .stopAndAdd(RaiseA())
                        .splineToLinearHeading(new Pose2d(-53,-40,Math.PI),Math.PI)
            ...
            else if (cv.propLoc== OpenCVB.PropLoc.Middle) {
            ...
            else if (cv.propLoc== OpenCVB.PropLoc.Right) {

                Actions.runBlocking(drive.mirroredActionBuilder(new Pose2d(-38, -64, Math.PI/2))

                        .splineToLinearHeading(new Pose2d(-31,-25,0),0)
                        //.stopAndAdd(Servo())
                        .waitSeconds(1.5)
                        .lineToX(-28)
                        .strafeTo(new Vector2d(-36,-58))
