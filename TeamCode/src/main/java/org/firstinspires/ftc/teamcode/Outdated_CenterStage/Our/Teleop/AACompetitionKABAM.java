...
@TeleOp(name="AACompetitionTELEOP", group="Linear OpMode")

@Disabled

public class AACompetitionKABAM extends LinearOpMode {
    ...
    private void GLM (){
        //THe RSM is 1 (360 degrees) so it is too big and we need to decrease it
        //double is for variables with numbers, boolean is TF
        //private void is a function that runs whenever you place the title
        double wristmultiply = .01;
        double RSM = -gamepad2.right_stick_y;
        double WP = wrist.getPosition();
        wrist.setPosition(WP+(RSM*wristmultiply));
        //When we press the LT, the wrist will move immediately to a position of 90 degrees
        boolean BL = gamepad2.b;
        if (BL) {
